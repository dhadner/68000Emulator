...
namespace PendleCodeMonkey.MC68000EmulatorLib
{
    ...
    public partial class Machine
    {
        ...
        internal class OpcodeExecutionHandler
        {
            ...
            private Dictionary<OpHandlerID, OpHandler> _handlers = new Dictionary<OpHandlerID, OpHandler>();
            ...
            private void WriteEAValue(Instruction instruction, uint value, EAType eaType)
            {
                ...
                else if (address.HasValue)
                {
                    ...
                    {
                        case OpSize.Byte:
                            Machine.Memory.WriteByte(address.Value, (byte)value);
                            break;
                        case OpSize.Word:
                            Machine.Memory.WriteWord(address.Value, (ushort)value);
                            break;
                        default:
            ...
            internal (byte? dataRegNum, byte? addrRegNum, uint? address, uint? immValue) EvaluateEffectiveAddress(Instruction instruction, EAType eaType, bool suppressIncDec = false)
            {
                ...
                if (address.HasValue)
                {
                    // Support trap handling in subclasses
            ...
            private TrapException? DIVU(Instruction inst)
            {
                ...
                if (value.HasValue)
                {
                    ...
                    var remainder = dRegVal % value.Value;
            ...
            private TrapException? BTST_BCHG_BCLR_BSET(Instruction inst)
            {
                ...
                var (_, _, address, _) = EvaluateEffectiveAddress(inst, EAType.Destination, !isBTST);
                if (address.HasValue)
                {
                    bitNum &= 0x00000007;
